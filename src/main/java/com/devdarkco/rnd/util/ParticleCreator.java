    ...
    public static void spawnParticle(EnumParticleTypes particle, World worldIn, BlockPos pos, int amount, Random rnd)
    {
        ...
        else
        {
            for (int i1 = 0; i1 < amount; ++i1)
            {
                ...
                double d1 = rnd.nextGaussian() * 0.02D;
                double d2 = rnd.nextGaussian() * 0.02D;
                worldIn.spawnParticle(EnumParticleTypes.VILLAGER_HAPPY, (double)((float)pos.getX() + rnd.nextFloat()), (double)pos.getY() + (double)rnd.nextFloat() * 1.0f, (double)((float)pos.getZ() + rnd.nextFloat()), d0, d1, d2, new int[0]);
            }
        }
    }
}
